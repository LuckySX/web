 ...
 * @LastEditors: fuping
 * @LastEditTime: 2020-06-24 11:44:33
 -->
<template>
  <div :class="regionStatistics.main">
    ...
    <div :class="regionStatistics.contentBox">
      ...
      <div :class="regionStatistics.barBox">
        <BorderBox title="" type="table">
          <VTable :option="tableOpt" :height="tableHeight" ref="table" v-if="showTable"></VTable>
        </BorderBox>
      </div>
    </div>
  </div>
...
const UrlRegionLine = '/areaNew/diagram'
const UrlRegionTable = '/areaNew/tableForWebIP'
...
export default class RegionStatistics extends Vue {
  ...
  sortChange(column: object) {
    ...
    if (column[name] === 'descending') {
      this.tableValue.order = 'desc'
    } else {
      this.tableValue.order = 'asc'
    }
    this.tableValue.sort = name
    this.getTableData()
  }
  getAllData() {
    // 折线图
  ...
  private changeOpt(index, value) {
    ...
    if (value === 1) {
      ...
      this.tableValue.sort = 'usernum'
    } else {
      this.tableOpt = this.tableWeb
      this.tableValue.sort = 'webnum'
    }
