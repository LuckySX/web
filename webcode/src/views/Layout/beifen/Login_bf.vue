 ...
 * @LastEditors: fuping
 * @LastEditTime: 2020-05-07 14:00:20
 * @Description: 
 -->
<template>
...
export default class Login extends Vue {
      ...
      .then((res: any) => {
        ...
        switch (result) {
          ...
          case LOGIN_STATUS.SUCCESS:
            // setCookies('SZToken', res.data.token)
            setSession({ SZToken: res.data.token })
            this.SET_TOKEN({ token: res.data.token })
  ...
  checkImageCode(code: string, password: string) {
    http
      .post<IResponse>('/imageCode/checkImage', {
        checkCode: code,
        password: this.md5Judge(password)
      })
      .then((res: any) => {
