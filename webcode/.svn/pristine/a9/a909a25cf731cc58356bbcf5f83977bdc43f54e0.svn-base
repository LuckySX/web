...
<template>
  <div :class="pageBoxOne.main" class="sz">
    <el-table :data="tableDataThree" stripe style="width: 100%" height="90%">
      <el-table-column prop="nationalname" label="国家" show-overflow-tooltip></el-table-column>
      <el-table-column prop="attackflow" label="攻击流量" show-overflow-tooltip></el-table-column>
      <!-- <el-table-column prop="proportion" label="流量占比" width="70px" show-overflow-tooltip></el-table-column> -->
    </el-table>
...
export default class BorderCon extends Vue {
  ...
  private prevPage() {
    if (this.curPage !== 1 && this.curPage !== 0) {
      this.curPage--
      this.tableDataThree = this.tableData.slice((this.curPage - 1) * this.pageSize, this.curPage * this.pageSize)
    }
  }
  private nextPage() {
  ...
  private loadData() {
    ...
    this.tableDataThree = this.tableData.slice(0, this.pageSize)
  }
  mounted() {
    this.loadData()
  }
