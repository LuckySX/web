 ...
 * @LastEditTime: 2020-07-22 09:07:58
            ...
            <el-form :inline="true" :model="tableformItems" label-position="left" label-width="100px">
              ...
              <el-form-item label="疑似攻击类型：">
                <el-select v-model="tableformItems.attackType" placeholder="请选择">
                  <el-option v-for="item in attackOpt" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="危险等级程度：">
                <el-select v-model="tableformItems.rank" placeholder="请选择">
                  <el-option v-for="item in riskOpt" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="handleTableQueryBtn">查询</el-button>
                <el-button type="primary" @click="handleTableResetBtn" style="display: none">
                  重置
                </el-button>
...
export default class Overall extends Vue {
  ...
  // 表格查询按钮
  handleTableQueryBtn() {
    this.tableformItems.primaryPort = _.trim(this.tableformItems.primaryPort)
  ...
  handleTableExportBtn() {
    if (this.exportType === 0) {
      ...
      window.location.href =
        process.env.VUE_APP_BASE_API + `/newDDoS/excelExport?${this.getParmas(this.tableformItems)}&type=1&eIp=${this.multipleSelection}`
    }
  }
  // 表格多选
  ...
  // 获取柱图数据
  getBarData() {
    http.post<IParams>('/newDDoS/getIpTop5', this.formItems).then((resp: any) => {
      const { data } = resp
      if (data.length === 0) {
        data.push({
          name: '无数据',
  ...
  getScatterData() {
    ...
    http.post<IParams>('/newDDoS/getScatterPointbyIp', params).then((resp: any) => {
      const { data } = resp
      const chartData: any = []
      data.forEach((i: any) => {
  ...
  // 获取表格数据
  getTableData() {
    http.post<IParams>('/newDDoS/getDDoSList', this.tableformItems).then((resp: any) => {
      const {
        data: { rows, total }
      } = resp
      if (rows.length > 0) {
